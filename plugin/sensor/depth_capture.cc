...
namespace mujoco::plugin::sensor
{
    ...
    DepthCapture* DepthCapture::Create(const mjModel* m, mjData* d, int instance)
    {
        ...
        if (max_distance_str && CheckAttr(std::string(max_distance_str))) {
            ...
            std::cout << "[DepthCapture] Create: max_distance = " << max_distance << std::endl;
        } else {
            std::cout << "[DepthCapture] Create: max_distance not specified or invalid, using default = " << max_distance << std::endl;
        }
        ...
        std::cout << "[DepthCapture] Create: All parameters validated successfully" << std::endl;
        std::cout << "[DepthCapture] Create: Creating DepthCapture with ncol=" << ncol
                  << ", nrow=" << nrow << ", fov_x=" << fov_x << ", fov_y=" << fov_y
    ...
    {
        std::cout << "[DepthCapture] Constructor: Starting initialization for instance " << instance << std::endl;
        ...
        for (int i = 0; i < m->nsensor; ++i) {
            if (m->sensor_type[i] == mjSENS_PLUGIN && m->sensor_plugin[i] == instance) {
                sensor_id = i;
                std::cout << "[DepthCapture] Constructor: Found sensor id = " << sensor_id << std::endl;
                ...
                std::cout << "[DepthCapture] Constructor: Sensor attached to site id = " << site_id << std::endl;
                break;
            }
        }
        ...
        std::cout << "[DepthCapture] Constructor: Pre-computing " << nrays << " ray directions" << std::endl;
        ...
        std::cout << "[DepthCapture] Constructor: Initialization complete" << std::endl;
    }

    void DepthCapture::Reset(const mjModel* m, int instance)
    {
        std::cout << "[DepthCapture] Reset: instance " << instance << std::endl;
        // No state to reset
    }

    void DepthCapture::Compute(const mjModel* m, mjData* d, int instance)
    {
        std::cout << "[DepthCapture] Compute: Starting computation for instance " << instance << std::endl;
        ...
        std::cout << "[DepthCapture] Compute: Using sensor id = " << id << std::endl;
        ...
        std::cout << "[DepthCapture] Compute: Site id = " << site_id
                  << ", pos = [" << site_pos[0] << ", " << site_pos[1] << ", " << site_pos[2] << "]" << std::endl;
        std::cout << "[DepthCapture] Compute: Performing raycasting for " << nrays << " rays" << std::endl;
        ...
        std::cout << "[DepthCapture] Compute: Raycasting complete. Hits: " << hits << "/" << nrays << std::endl;
    ...
    void DepthCapture::Visualize(const mjModel* m, mjData* d,
                                  ...
                                  int instance)
    {
        std::cout << "[DepthCapture] Visualize: instance " << instance << std::endl;
        // Optional visualization
    }

    void DepthCapture::RegisterPlugin()
    {
        std::cout << "[DepthCapture] RegisterPlugin: Starting plugin registration" << std::endl;
        ...
        plugin.nstate = +[](const mjModel* m, int instance) {
            std::cout << "[DepthCapture] nstate callback: instance " << instance << std::endl;
            return 0;
        };

        plugin.nsensordata = +[](const mjModel* m, int instance, int sensor_id) {
            std::cout << "[DepthCapture] nsensordata callback: instance " << instance << ", sensor_id " << sensor_id << std::endl;
            ...
            int total = ncol * nrow;
            std::cout << "[DepthCapture] nsensordata: returning " << total << " (ncol=" << ncol << ", nrow=" << nrow << ")" << std::endl;
            return total;
        };
        ...
        plugin.init = +[](const mjModel* m, mjData* d, int instance) {
            std::cout << "[DepthCapture] init callback: instance " << instance << std::endl;
            ...
            std::cout << "[DepthCapture] init callback: Successfully initialized instance " << instance << std::endl;
            return 0;
        };

        plugin.destroy = +[](mjData* d, int instance) {
            std::cout << "[DepthCapture] destroy callback: instance " << instance << std::endl;
        ...
        plugin.reset = +[](const mjModel* m, mjtNum* plugin_state, void* plugin_data,
                           int instance) {
            std::cout << "[DepthCapture] reset callback: instance " << instance << std::endl;
        ...
        plugin.compute =
            +[](const mjModel* m, mjData* d, int instance, int capability_bit) {
                std::cout << "[DepthCapture] compute callback: instance " << instance << ", capability_bit " << capability_bit << std::endl;
                auto* depthCapture =
        ...
        plugin.visualize = +[](const mjModel* m, mjData* d, const mjvOption* opt,
            ...
            std::cout << "[DepthCapture] visualize callback: instance " << instance << std::endl;
            auto* depthCapture =
                reinterpret_cast<class DepthCapture*>(d->plugin_data[instance]);
        ...
        std::cout << "[DepthCapture] RegisterPlugin: Plugin registered successfully" << std::endl;
    }
